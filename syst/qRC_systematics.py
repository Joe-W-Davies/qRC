...
class utils(object):
    ...
    def clf_reweight(df_mc,df_data,n_jobs=1,cut=None):
        ...
        if cut is not None:
            #take random 1m events as a X sample
        ...
        clf.fit(X,y)
        eps = 1.e-3
        #take the ratio of p_data and p_mc, predicted on entire mc set, as correction (eps so non inf values)
